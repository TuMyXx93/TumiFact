...
export class InventarioRepository {
  async getMovimientos(limit = 50, productoId?: number) {
    ...
    if (productoId) {
      return await query
        .where(eq(movimientosInventario.producto_id, productoId))
  ...
  async getStockCritico() {
    return await db
      ...
      .from(productos)
      .where(
        sql`CAST(${productos.stock_actual} AS NUMERIC) <= CAST(${productos.stock_minimo} AS NUMERIC)`
      )
  ...
  async findByIdempotencyKey(key: string): Promise<MovimientoInventarioItem | null> {
    const rows = await db
      .select()
      .from(movimientosInventario)
      .where(eq(movimientosInventario.idempotency_key, key))
      .limit(1);
    return rows[0] || null;
  }
}
