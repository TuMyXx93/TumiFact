...
categoriasRouter.post('/', verifyAuth, requireRole('gerente', 'admin'), validateDTO(CreateCategoriaDTO), async (req, res, next) => {
  try {
...
categoriasRouter.put('/:id', verifyAuth, requireRole('gerente', 'admin'), validateDTO(UpdateCategoriaDTO), async (req, res, next) => {
  try {
    const id = parseInt(String(req.params.id), 10);
