  ...
  async (req, res, next) => {
    try {
      ...
      if (format === 'csv') {
        ...
        res.setHeader(
          'Content-Disposition',
          `attachment; filename="ventas_tumifact_${new Date().toISOString().split('T')[0]}.csv"`
        );
        return res.send(csv);
      }
      ...
      if (format === 'pdf') {
        ...
        res.setHeader(
          'Content-Disposition',
          `inline; filename="reporte_ventas_${new Date().toISOString().split('T')[0]}.pdf"`
        );
...
reportesRouter.get(
  ...
  async (req, res, next) => {
    try {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="inventario_tumifact_${new Date().toISOString().split('T')[0]}.csv"`
      );
      res.send(csv);
    } catch (error) {
      next(error);
...
reportesRouter.get(
  ...
  async (req, res, next) => {
    try {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="separados_tumifact_${new Date().toISOString().split('T')[0]}.csv"`
      );
      res.send(csv);
    } catch (error) {
      next(error);
