import { pgTable, serial, varchar, text, integer, timestamp, jsonb, customType } from 'drizzle-orm/pg-core';
...
export const configuracionImpresion = pgTable('configuracion_impresion', {
  ...
  esquema_colores: jsonb('esquema_colores').default({
    primary: '#2563eb',
    secondary: '#0891b2',
    accent: '#10b981',
    background: '#0b0f19',
  ...
  mensaje_bienvenida: varchar('mensaje_bienvenida', { length: 200 }).default('¡Gracias por su compra!'),
  mensaje_pie: text('mensaje_pie').default('¡Vuelva pronto!'),
  politica_devolucion: text('politica_devolucion').default('Cambios y devoluciones dentro de los 30 días con el comprobante de compra.'),
  politica_separados: text('politica_separados').default('Plazo máximo de separado: 30 a 45 días. Abonos no reembolsables si vence el plazo.'),
